...
fluent_scores = [x[sys.argv[3]] for x in all_scores]
...
ner_weight, qt_weight, re_weight, d = np.mean(ner), np.mean(qt), np.mean(re), np.mean(d)
...
print (np.mean(fluent_scores), np.mean(new_scores))
np.savetxt('refscores', fluent_scores)
np.savetxt('goldscores',new_scores)
"""
a = ref_scores
